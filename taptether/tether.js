...
class Tether {
  ...
  async title() {
    ...
    console.log(color.yellow("Telegram: @zepmo\n"));
  }
    ...
    while (true) {
      ...
      for (let i = 0; i < datas.length; i++) {
        ...
        try {
            ...
            console.log(`========== Tài khoản ${i + 1}/${datas.length} | ip: ${proxyIP} ==========`);
            ...
            if (res?.data === undefined || res.data.success !== true) {
              ...
              const balance = await this.getBalance(proxy);
                console.log(
                    color.green(
                    `Số dư USDT: ${balance.balanceUSDT/1000000} | Số dư Gold: ${balance.balanceGold/1000000}`
                    )
                );
            }
        } catch (error) {
