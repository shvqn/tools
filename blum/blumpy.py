...
while True:
    ...
    for giây in range(thời_gian_chờ, 0, -1):
        sys.stdout.write(f"\r{Fore.CYAN}Chờ thời gian nhận tiếp theo trong {Fore.CYAN}{Fore.WHITE}{giây // 3600} giờ {Fore.WHITE}{(giây % 3600) // 60} phút {Fore.WHITE}{(giây % 3600) % 60} giây")
        sys.stdout.flush()
        time.sleep(1)
