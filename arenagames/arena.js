...
async function claimTask(stt, token, axios, idTask) {
	try {
		...
		console.log(`[*] Account ${stt} | claimTask: ${e}`);
	}
}
...
async function main(stt, account, axios)
{
    try{
        ...
        let infoAcc = await profile(stt, token, axios);
				...
				let expFarm = await getArena(stt, token, axios, 'https://bot.arenavs.com/v1/profile/exp-farm-coin');
				await sleep(randomInt(1,4));
				let urlAll = await getArena(stt, token, axios, 'https://bot.arenavs.com/v1/stat/users/all');
    ...
    for (const chunk of accountChunks) {
        ...
        const tasks = chunk.map(async (account, index) => {
			...
			if (account) {
					...
					const maskedProxy = proxy?.slice(0, -10) + '*c*u*a*d*e*v*';
